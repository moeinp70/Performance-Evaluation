...
# Parameters
srv_weibull_k, srv_weibull_lambda = 0.333, 2.5
arr_lambda1, arr_lambda2, arr_p1 = 0.025, 0.1, 0.35
srv_uniform_a, srv_uniform_b = 1, 10
arr_erlang_k, arr_erlang_lambda = 8, 1.25
...
def run_scenario(arrival_func, service_func, scenario_name):
    K = K0
    U1, U2, Th1, Th2, NJ1, NJ2, RT1, RT2 = 0, 0, 0, 0, 0, 0, 0, 0
    ...
    while K <= MaxK:
        ...
        # Stopping criteria
        if (utilization_rel_err < max_rel_err and
            throughput_rel_err < max_rel_err and
            avg_jobs_rel_err < max_rel_err and
            avg_response_time_rel_err < max_rel_err):
            break
    ...
    print(f"Utilization in {utilization_ci},  Relative Error: {utilization_rel_err}")
    print(f"Throughput in {throughput_ci},  Relative Error: {throughput_rel_err}")
    print(f"Average #jobs in {avg_jobs_ci},  Relative Error: {avg_jobs_rel_err}")
    print(f"Average Resp. Time in {avg_response_time_ci},  Relative Error: {avg_response_time_rel_err}")
    print("=========================================")
